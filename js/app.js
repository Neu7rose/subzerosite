(function () {
  ...
  const API = {
    searchUrl: 'https://api.magnatamusicas.site/search',
    playUrl: 'https://api.magnatamusicas.site/play',
    playBassUrl: 'https://api.magnatamusicas.site/playbass',
    downloadUrl: 'https://api.magnatamusicas.site/download',
  ...
  function normalizeMusic(item) {
    ...
    const thumbnailFromList = Array.isArray(thumbnails)
      ? [...thumbnails].reverse().find((entry) => typeof entry === 'string' || entry?.url)
      : null;
    ...
    return {
      ...
      thumbnail: source.thumbnail?.url || source.thumbnail || source.thumb || source.image?.url || source.image || source.cover?.url || source.cover || source.capa || (typeof thumbnailFromList === 'string' ? thumbnailFromList : thumbnailFromList?.url),
      url: source.url || source.webpage_url || source.youtubeUrl || source.youtube_url || source.link || item.url
    };
  }

  function getTitle(music) {
    return music?.title || music?.name || music?.nome || music?.titulo || music?.videoTitle || music?.snippet?.title || 'Música sem título';
  }

  function getArtist(music) {
    const author = music?.artist || music?.author || music?.channel || music?.channelTitle || music?.canal || music?.uploader || music?.artista || music?.snippet?.channelTitle;
    return typeof author === 'object' ? (author.name || author.title || 'Artista desconhecido') : (author || 'Artista desconhecido');
  }

  function getYoutubeId(music) {
    const rawId = music?.id || music?.videoId || music?.video_id || music?.youtubeId || music?.youtube_id || music?.ytid || music?.identifier;
    const direct = typeof rawId === 'object' ? (rawId.videoId || rawId.id) : rawId;
    if (direct) return String(direct);

    const url = music?.url || music?.link || music?.youtube || music?.youtubeUrl || music?.youtube_url || music?.webpage_url;
    if (!url) return '';
  ...
  function getCover(music) {
    const id = getYoutubeId(music);
    const cover = music?.thumbnail?.url || music?.thumbnail || music?.image?.url || music?.image || music?.cover?.url || music?.cover || music?.thumb || music?.capa;
    return cover || (id ? `https://i.ytimg.com/vi/${id}/hqdefault.jpg` : fallbackCover);
  }
